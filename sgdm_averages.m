...
for r=1:runs
    ...
    for i=1:(niter/(n*cycles_per_perm))
        %MSE_SGDM_SO_r = MSE_SGDM_SO_r + mean(iterates_dist_SGDM_SO(((i-1)*n*cycles_per_perm + mean_first):(i*n*cycles_per_perm)).^2);
    ...
    %MSE_SGDM_SO(r) = MSE_SGDM_SO_r/(niter/(n*cycles_per_perm));
end
mean_MSE_SGDM_SO = mean(MSE_SGDM_SO)
%std_MSE_SGDM_SO = std(MSE_SGDM_SO)/sqrt(runs)
std_MSE_SGDM_SO = std(MSE_SGDM_SO)/sqrt(runs*niter/(n*cycles_per_perm))

%SGDM_SO_approx
%MSE_SGDM_SO_approx = zeros(1,runs);
...
for r=1:runs
    ...
    %MSE_SGDM_SO_approx_r = 0;
    for i=1:(niter/(n*cycles_per_perm))
        %MSE_SGDM_SO_approx_r = MSE_SGDM_SO_approx_r + mean(iterates_dist_SGDM_SO_approx(((i-1)*n*cycles_per_perm + mean_first):(i*n*cycles_per_perm)).^2);
    ...
    %MSE_SGDM_SO_approx(r) = MSE_SGDM_SO_approx_r/(niter/(n*cycles_per_perm));
end
